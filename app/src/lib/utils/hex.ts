// Encode and return as hexadecimal string
export const hexEncode = (data: ArrayBuffer | string): string => {
  const uint8 =
    typeof data === 'string'
      ...
      ? data
      : new Uint8Array(data);
  return new TextDecoder().decode(encode(uint8));
};

// Decode from hexadecimal string
export const hexDecode = (data: string): Uint8Array => {
